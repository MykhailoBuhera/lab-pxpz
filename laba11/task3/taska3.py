...
def float_range(start: float, stop: float, step: float) -> Iterator[float]:
    ...
    # Використовуємо decimal для уникнення проблем з точністю
    # float. Це особливо важливо для операцій порівняння та
    # додавання дробових чисел.
    # Ми визначаємо точність (precision) на основі кроку,
    # щоб коректно обробляти випадки на кшталт 0.1.
    try:
